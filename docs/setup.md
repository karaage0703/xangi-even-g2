...
- 上下スクロール: セッション選択
- Discord セッションは通常チャンネルを `#チャンネル名`、スレッドを `T: スレッド名` で表示
- タップ: 選択セッションを開く
...
- 上下スクロール: 履歴・ページ移動
- 履歴の端まで移動すると、前後の履歴を30件ずつ追加取得
- 最初のページから前へ移動すると最新へ、最後のページから次へ移動すると最初へ循環
- xangi が生成した回答候補がある場合は、会話本文の末尾に続く追加ページとして自動表示（固定候補は表示しない）
- タップ: 録音開始
